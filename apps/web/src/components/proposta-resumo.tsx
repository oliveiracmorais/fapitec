...
export default function PropostaResumo({ dadosProponente, dadosAcademicos, itensOrcamentarios, status, protocolo, valorGlobalEdital }: Props) {
  ...
  return (
    <div className="divide-y divide-gray-100">
      <h2 className="pb-4 text-lg font-semibold text-gray-900">Resumo da Proposta</h2>
          ...
          <div className="space-y-1">
            {itensOrcamentarios.map((item, i) => (
              <div key={i} className="flex flex-col gap-1 text-sm sm:flex-row sm:justify-between">
                ...
                <span className="font-medium text-gray-900 sm:text-right">{formatarBRL(item.valor_total)}</span>
              </div>
            ))}
